...
class CommonController extends Controller
{
    ...
    public function view_index()
	{
	   ...
	   $data = array('blog' => $blog, 'comments' => $comments, 'review' => $review, 'totalmembers' => $totalmembers, 'totalsales' => $totalsales, 'totalfiles' => $totalfiles,'total_services' => $total_services, 'totalearning' => $totalearning, 'featured' => $featured, 'newest' => $newest, 'free' => $free, 'popular' => $popular, 'flash' => $flash);
   ...
   /* public function user_verify($user_token)
	  ...
	  { 
			  ...
			  else
			  {
				if($type == 'category')
				{
				   ...
				   $itemData['item'] = Items::with('ratings')->leftjoin('users', 'users.id', '=', 'items.user_id')->where('users.user_subscr_date','>=',$today)->where('items.item_status','=',1)->where('items.drop_status','=','no')->where('items.item_category_parent','=',$category_id)->orderBy('items.item_id', 'desc')->get();
				   }
				   else
				   {
				     return view('/404');
	         ...
	         if($type == 'item-type')
			  ...
			  else
			  {
				if($type == 'category')
				{
				   ...
				   $category_data = Category::getcategorysingle($slug);
				   $category_id = $category_data->cat_id;
				   $itemData['item'] = Items::with('ratings')->leftjoin('users', 'users.id', '=', 'items.user_id')->where('items.item_status','=',1)->where('items.drop_status','=','no')->where('items.item_category_parent','=',$category_id)->orderBy('items.item_id', 'desc')->get();
				   }
				   else
				   {
				     return view('/404');
	...
	public function view_shop_items(Request $request)
	{
	  ...
	  if($search_keyword != "" ||  $orderby != "" || $min_price != "" || $max_price != "")
	  {
		  ...
		  if($additional['settings']->subscription_mode == 1)
		  {
			  $itemData['item'] = Items::with('ratings')
							  ...
							  ->where(function ($query) use ($search_keyword,$orderby,$min_price,$max_price,$item_types,$category_names) { 
