<div class="page-title-overlap pt-4" style="background-image: url('{{ url('/') }}/public/storage/settings/{{ $allsettings->site_banner }}');">
      <div class="container d-flex flex-wrap flex-sm-nowrap justify-content-center justify-content-sm-between align-items-center pt-2">
        <div class="media media-ie-fix align-items-center pb-3">
          ...
          <img class="rounded-circle" src="{{ url('/') }}/public/storage/users/{{ $user['user']->user_photo }}" alt="{{ $user['user']->username }}" width="90" height="90">
          @else
          <img src="{{ url('/') }}/public/img/no-user.png" alt="{{ $user['user']->username }}" width="90" height="90">
          @endif
          </div>
          <div class="media-body pl-3">
			...
			{{ $user['user']->username }}	  
			@endif			
			@if($addition_settings->subscription_mode == 1) @if($user['user']->user_document_verified == 1) <span class="badges-success"><i class="dwg-check-circle danger"></i> {{ Helper::translation(5202,$translate,'') }}</span> @endif @endif</h3>
